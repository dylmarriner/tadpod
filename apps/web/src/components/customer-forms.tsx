...
export function CustomerCreateForm() {
  ...
  async function checkSimilarNames(name: string): Promise<void> {
    ...
    try {
      setSimilarNames(await browserApi<{ id: string; code: string; name: string }[]>(`/customers/similar-names?name=${encodeURIComponent(name.trim())}`));
  ...
  return <form className="form-stack" onSubmit={submit}>
    ...
    {similarNames.length > 0 ? <div className="form-message" role="status">
      Possible duplicate: {similarNames.map((customer) => `${customer.code} — ${customer.name}`).join(', ')}. This is only a warning; you can still create this customer.
    </div> : null}
    ...
    <div className="grid grid--2">
      ...
      <Field label="Credit limit (0 = no limit)"><TextInput name="creditLimit" inputMode="decimal" defaultValue="0" /></Field>
    </div>
    <div className="grid grid--2">
    ...
    <Button disabled={mutation.busy}>Create customer</Button>
  </form>;
}

export function CustomerArchiveToggle({ customerId, active }: { customerId: string; active: boolean }) {
  const mutation = useMutation();
  return <Button variant={active ? 'danger' : 'secondary'} disabled={mutation.busy} onClick={() => void mutation.run(() => browserApi(`/customers/${customerId}`, { method: 'PATCH', body: JSON.stringify({ active: !active }) }))}>
...
export function CustomerAddressForm({ customerId }: { customerId: string }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>): void {
    ...
    const data = new FormData(form);
    void mutation.run(async () => {
      await browserApi(`/customers/${customerId}/addresses`, {
        method: 'POST',
        body: JSON.stringify({
          type: data.get('type'),
...
export function RemoveCustomerAddressButton({ customerId, addressId }: { customerId: string; addressId: string }) {
  const mutation = useMutation();
  return <Button variant="danger" disabled={mutation.busy} onClick={() => void mutation.run(() => browserApi(`/customers/${customerId}/addresses/${addressId}`, { method: 'DELETE' }))}>
    Remove
  </Button>;
}
