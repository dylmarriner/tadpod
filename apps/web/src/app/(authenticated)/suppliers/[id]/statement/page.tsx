...
type Statement = { supplierId: string; asOf: string; openingBalance: string; closingBalance: string; lines: readonly StatementLine[] };
...
export default async function SupplierStatementPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let loadError = '';
  try {
    [supplier, statement] = await Promise.all([serverApi<Supplier>(`/suppliers/${id}`), serverApi<Statement>(`/suppliers/${id}/statement`)]);
  } catch (caught) {
    loadError = caught instanceof ApiError ? caught.message : 'Could not load this statement.';
  }

  if (loadError || !supplier || !statement) return <div className="form-message" role="alert">{loadError || 'Statement not found.'}</div>;

  return <>
    <header className="page-header">
      <div>
        <h1>Statement — {supplier.code} — {supplier.name}</h1>
        <p>As of {new Date(statement.asOf).toLocaleString('en-NZ')} · Closing balance {statement.closingBalance}</p>
      </div>
      <a href={`${apiUrl}/documents/suppliers/${supplier.id}/statement`} target="_blank" rel="noreferrer">Print</a>
    </header>
    <Card title="Transactions">
      {statement.lines.length === 0
        ? <EmptyState title="Nothing to show" description="No bills, payments, or credits have been posted for this supplier yet." />
