...
export default async function StockCountsPage() {
  ...
  return <>
    <PageHeader
      ...
      actions={<Link className="button button--primary" href="/inventory/stock-counts/new">Start stock count</Link>}
    />
    <Card kicker="Count sessions" title="Stock counts">
      {loadError ? <div className="form-message" role="alert">{loadError}</div>
        : page === null || page.items.length === 0 ? <EmptyState title="No stock counts yet" description="Stock counts will appear here once started." action={<Link className="button button--primary" href="/inventory/stock-counts/new">Start the first stock count</Link>} />
        : <DataTable label="Stock counts" headings={['Started', 'Warehouse', 'Status', 'Progress', 'Started by', 'Posted', '']}>
            {page.items.map((item) => <tr key={item.id}>
              ...
              <td><Link className="button button--secondary" href={`/inventory/stock-counts/${item.id}`}>Open</Link></td>
            </tr>)}
          </DataTable>}
    </Card>
  </>;
